...
$require = array('姓','姓フリガナ','名','名フリガナ','メール');
...
<?php echo $errm; ?><br /><br /><input type="button" value=" 前画面に戻る "onClick="history.go(-1)">
</div>
<?php }else{ ?>
<h3>確認画面</h3>
...
<input type="button" value="前画面に戻る"onClick="history.go(-1)"></p>
</form>
<?php } ?>
</div><!-- /formWrap -->
<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
...
<br /><br /><input type="button" value=" 前画面に戻る ">
</div>
</body>
</html>
<?php }else{ ?>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" 前画面に戻る "></div>
<?php 
...
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="http://allcontents.info/hotel/daisysinnhowdy" target="_blank">- Daisy`s INN Howdy -</a>';
}
//----------------------------------------------------------------------
//  関数定義(END)
//----------------------------------------------------------------------
